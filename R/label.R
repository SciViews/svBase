...
`labelise.default` <- function(x, label, units = NULL, as_labelled = FALSE,
  ...
  if (!is.null(label) && !is.na(label)) {
    ...
    if (length(label) != 1L)
      stop("{.arg label} must be {.cls character} of length 1")
    attr(x, "label") <- label
...
`labelise.data.frame` <- function(x, label, units = NULL, as_labelled = FALSE,
  ...
  if (isTRUE(self)) {
    ...
    nc <- ncol(x)
    if (nc == 0) {
      warning("Impossible to label variables of a {.cls data.frame} with no columns")
      return(x)
    }
    vars <- names(x)
    ...
    if (is.null(lvars)) {
      if (length(label) != nc)
        stop("{.arg label} as unnamed {.cls list} must have the same length as {.arg x}")
      names(labels) <- vars
    } else {# Named list: all items must be named and match names in x
      missing_lvars <- lvars[!lvars %in% vars]
      if (length(missing_lvars))
        stop("The following label names are not in {.arg x}:",
          `*` = "{missing_lvars}")
      label <- label[vars]
      names(label) <- vars
    }
    if (is.null(units)) {
      ...
      stop("{.arg units} must be a {.cls list} or {.cls character} of same length as {.arg x}")
    } else {
      ...
      if (is.null(uvars)) {
        if (length(units) != nc)
          stop("{.arg units} as unnamed {.cls list} must have the same length as {.arg x}")
        names(units) <- vars
      } else {# Named list: all items must be named and match names in x
        missing_uvars <- uvars[!uvars %in% vars]
        if (length(missing_uvars))
          stop("The following units names are not in {.arg x}:",
            `*` = "{missing_uvars}")
        units <- units[vars]
        names(units) <- vars
      }
