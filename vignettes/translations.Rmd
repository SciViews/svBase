---
title: "Translations with 'svBase'"
...
output:
  rmarkdown::html_vignette:
    ...
    fig_caption: yes
vignette: >
  %\VignetteIndexEntry{Translations with 'svBase'}
...
The `ngettext()` function accepts only a fixed number of arguments, on the contrary to `gettext()` or `gettextf()`. We had to remain consistent with this. So, `ngettext_()` hacks the `domain =` argument to provide both the domain *and* the language. You must combine tboth like `domain/lang`. So, `NULL/fr` will use the default domain and French language, or `R-stats/de` uses a specified domain 'R-stats' and German language.
